...
export class DatabaseStorage implements IStorage {
  ...
  async getFinancialStanding(): Promise<{
    currentMonth: {
      totalReceivables: number;
      totalPayables: number;
      poInTransit: number;
    ...
    const [currentMonthMetrics, lastMonthMetrics] = await Promise.all([
      getMetricsForPeriod(currentMonth, currentYear, true),
      getMetricsForPeriod(lastMonth, lastMonthYear, false),
    ]);
